...
class AdminOnlyMixin:
    def dispatch(self, request, *args, **kwargs):
        ...
        # Memastikan user memiliki atribut is_admin (sesuai skenario Smart City)
...
class ReportUpdateStatusView(AdminOnlyMixin, View):
    def post(self, request, pk):
        ...
        if report.status in valid_transitions and new_status == valid_transitions[report.status]:
            ...
            messages.warning(request, "Perubahan status tidak valid atau tidak sesuai urutan ❌")
...
def dashboard_data(request):
    ...
    # Mengambil data untuk tabel (Menambahkan 'id' agar tombol detail berfungsi)
...
def search_reports(request):
    ...
    data = list(reports.values('id', 'title', 'category', 'status', 'location'))
...
def report_detail_api(request, pk):
    ...
    data = {
        ...
        'created_at': report.created_at.strftime("%d %b %Y %H:%M") if hasattr(report, 'created_at') else ""
    }
