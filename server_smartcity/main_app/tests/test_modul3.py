...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        ...
        """
        # ARRANGE: login sebagai pemilik laporan, siapkan payload perubahan judul
        self.client.force_authenticate(user=self.warga)
        url = f'/api/report/{self.laporan_reported.pk}/'
        payload = {
            'title': 'Judul Diubah Padahal Sudah REPORTED',
        ...
        # ACT: warga mencoba PUT mengubah konten
        response = self.client.put(url, payload, format='json')

        # ASSERT: ditolak dengan 403, karena status sudah bukan DRAFT
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Laporan berstatus REPORTED tidak boleh diedit oleh warga (403)"
        )

        # ASSERT: pastikan data di database tidak berubah
        self.laporan_reported.refresh_from_db()
        self.assertNotEqual(
            self.laporan_reported.title,
            payload['title'],
            "Judul laporan REPORTED seharusnya tidak berubah"
        )
    ...
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        ...
        """
        # ARRANGE: login sebagai pemilik laporan yang sudah RESOLVED
        self.client.force_authenticate(user=self.warga)
        url = f'/api/report/{self.laporan_resolved.pk}/'
        payload = {
            'title': 'Mencoba Mengubah Laporan Resolved',
        ...
        # ACT: kirim PUT untuk memodifikasi laporan RESOLVED
        response = self.client.put(url, payload, format='json')

        # ASSERT: ditolak dengan 403 (read-only, terkunci mutlak)
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Laporan berstatus RESOLVED bersifat read-only (403)"
        )
        ...
        self.assertNotEqual(
            ...
            payload['title'],
            "Judul laporan RESOLVED seharusnya tidak berubah"
        )
...
class AdminWorkflowTests(TestCase):
    ...
    def test_WF_03_admin_mengubah_status_reported_ke_verified(self):
        ...
        """
        # ARRANGE: login sebagai admin via session (Django monolitik)
        self.client.login(username='admin_portal', password='AdminPass123!')

        # ACT: POST ke endpoint update-status untuk mengubah REPORTED -> VERIFIED
        url = reverse('update_status', args=[self.laporan_reported.pk])
        response = self.client.post(url, {'status': 'VERIFIED'})

        # ASSERT: view melakukan redirect setelah sukses (302 ke report_list)
        self.assertEqual(
            response.status_code,
            status.HTTP_302_FOUND,
            "Setelah update status, admin seharusnya di-redirect (302)"
        )
        ...
        self.assertEqual(
            ...
            'VERIFIED',
            "Status laporan seharusnya berubah dari REPORTED menjadi VERIFIED"
        )
    ...
    def test_WF_04_tidak_ada_transisi_langsung_ke_resolved_dari_reported(self):
        ...
        """
        # ARRANGE: login sebagai admin, laporan berstatus REPORTED (dari setUp)
        self.client.login(username='admin_portal', password='AdminPass123!')

        # ACT: admin mencoba "lompat" langsung ke RESOLVED, melewati
        # VERIFIED dan IN_PROGRESS
        url = reverse('update_status', args=[self.laporan_reported.pk])
        response = self.client.post(url, {'status': 'RESOLVED'})

        # ASSERT: view tetap redirect (baik sukses maupun gagal, selalu
        # redirect ke report_list), tapi...
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)

        # ASSERT: status di database TIDAK berubah menjadi RESOLVED,
        # karena REPORTED -> RESOLVED bukan transisi valid pada state machine
        self.laporan_reported.refresh_from_db()
        self.assertNotEqual(
            self.laporan_reported.status,
            'RESOLVED',
        ...
        self.assertEqual(
            self.laporan_reported.status,
            'REPORTED',
            "Status laporan seharusnya tetap REPORTED karena transisi ditolak"
        )