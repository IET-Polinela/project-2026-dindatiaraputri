...
class AuthenticationTests(APITestCase):
    ...
    def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
        ...
        """
        # ARRANGE: login sebagai warga biasa menggunakan session auth
        # (portal admin memakai Django session, bukan JWT)
        self.client.login(username='tiara', password='dindacantik')

        # ACT: warga mencoba mengakses halaman dashboard admin
        url = reverse('dashboard')
        response = self.client.get(url)

        # ASSERT: sistem harus menolak dan meredirect (302), bukan 200
        self.assertEqual(
            response.status_code,
            status.HTTP_302_FOUND,
            "Warga biasa (non-staff) seharusnya di-redirect (302) saat "
