...
class CRUDAndValidationTests(APITestCase):
    ...
    def test_FT_01_buat_laporan_dengan_data_lengkap(self):
        ...
        payload = {
            ...
            'description': 'Jalan berlubang sepanjang 2 meter',
            'location': 'Jl. Merdeka',
        }
        ...
        self.assertTrue(
            Report.objects.filter(title='Jalan Rusak').exists(),
            "Laporan baru seharusnya tersimpan di database"
        )

        laporan = Report.objects.get(title='Jalan Rusak')
        self.assertEqual(
            laporan.reporter,
            self.warga,
            "Field reporter seharusnya otomatis terisi dari user yang login"
        )
    ...
    def test_FT_02_ditolak_jika_judul_kosong(self):
        ...
        self.assertEqual(
            ...
            "Laporan tanpa title seharusnya ditolak (400)"
        )
        self.assertIn(
            'title',
            response.data,
            "Response error harus menyebutkan field 'title' sebagai penyebab"
        )
    ...
    def test_FT_03_ditolak_jika_deskripsi_kosong(self):
        ...
        self.assertEqual(
            response.status_code,
            status.HTTP_400_BAD_REQUEST,
            "Laporan tanpa description seharusnya ditolak (400)"
        )
        self.assertIn(
            'description',
            response.data,
            "Response error harus menyebutkan field 'description' sebagai penyebab"
        )
