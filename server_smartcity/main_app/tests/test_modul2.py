...
class PrivacyAndDataHidingTests(APITestCase):
    ...
    def test_PRIV_03_tidak_bisa_baca_draf_orang_lain(self):
        ...
        # ASSERT: sistem berpura-pura data tidak ada -> 404
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Warga A tidak boleh bisa melihat draf milik Warga B; "
            "seharusnya mendapat 404 Not Found"
        )
    ...
    def test_PRIV_04_tidak_bisa_modifikasi_draf_orang_lain(self):
        ...
        """
        # ARRANGE: login sebagai Warga A, siapkan payload modifikasi
        self.client.force_authenticate(user=self.warga_a)
        ...
        # ASSERT: request ditolak dengan 404
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Warga A tidak boleh bisa memodifikasi draf milik Warga B; "
            "seharusnya mendapat 404 Not Found"
        )

        # ASSERT: pastikan data asli di database tidak berubah sama sekali
        self.draft_milik_b.refresh_from_db()
        self.assertEqual(
            self.draft_milik_b.title,
            judul_asli,
            "Judul draf milik Warga B seharusnya tetap sama, tidak berubah"
        )